...
#------------------------------------------------------------------------------#
#    Fuente: Cuenta Pública (SHCP) | https://www.cuentapublica.hacienda.gob.mx/
#    Responble: Ytzel Maya | Coordinadora de Comunicación de Datos, OXFAM MX
#    Fecha de última actualización: 29 de noviembre de 2024 
#------------------------------------------------------------------------------#

# Objetivo principal: Analizar, de forma pedagógica y transparente, las bases de 
# datos de Cuenta Pública (SHCP) para la realización de análisis presupuestales.

# Objetivo de este script: Extracción y limpieza de base de datos correspondiente 
# a la Cuenta Pública, de 2013 a 2023 (solo a partir del año 2013 esta información 
# se encuentra desglosada de manera detallada)

#-------------------------------------------------------------------------------
# 00. Configuración inicial ---------------------------------------------------#
#-------------------------------------------------------------------------------

# Nota: Es necesario instalar los paquetes  si no has utilizado estas librerías
# anteriormentes
...
require(pacman)
pacman::p_load(dplyr, purrr, tidyr, readxl, readr)

#-------------------------------------------------------------------------------
# 01. Funciones y limpieza ----------------------------------------------------#
#-------------------------------------------------------------------------------

# Nota: Esta la parte más tardada. Son varios datos. Ten paciencia. La siguiente
# función te irá dando certezas sobre cómo se están bajando las bases de datos.

###--- Creamos una función para extraer los csv. Los loops nos van a permitir 
# guardar los csv incluso si cambian de patrones en los links.

# Función principal 
obtener_cuenta_publica <- function(anio) {
  # Definimos la URL base dentro de la función
  base_url <- "https://www.transparenciapresupuestaria.gob.mx/work/models/PTP/DatosAbiertos/BD_Cuenta_Publica/CSV/"
  
  # Lista de  patrones de nombres de archivo
  patrones <- list(
    paste0("Cuenta_Publica_", anio, ".csv"),
    paste0("cuenta_publica_", anio, "_ra_ecd.csv"),
    paste0("cuenta_publica_", anio, "_ra_ecd_epe.csv"),
    paste0("cuenta_publica_", anio, "_gf_ecd_epe.csv")
  )
  
  # Descarga del archivo. Usamos cada patrón de nombre de archivo
  for (patron in patrones) {
    url <- paste0(base_url, patron)
    
    tryCatch({
      message("Intentando descargar datos para el año ", anio, " desde ", url)
      data <- read.csv(url, na.strings = c("#N/A", "NA"), stringsAsFactors = FALSE, 
                       # Importanre: usamos este encoding para poder leer los
                       # caracteres del español
                       header = TRUE, sep = ",", encoding = "latin1")
      message("Descarga exitosa para el año ", anio, " con el patrón: ", patron)
      return(data)
    }, error = function(e) {
      message("No se pudo descargar el archivo con el patrón: ", patron, ". Error: ", e)
    })
  }
  
  # Si ninguno de los patrones funcionó, devolver NULL
  message("No se pudo descargar datos para el año ", anio, " después de intentar todos los patrones.")
  return(NULL)
}

# Creamos una lista para almacenar todos los datos
lista_cuentas_publicas <- list()

# Descargamos los archivos para los años 2013 a 2023 y los almacenamos en la lista
# (¡Este proceso puede demorar un poco porque son varias bases con mucha info!)
for (anio in 2013:2023) {
  lista_cuentas_publicas[[as.character(anio)]] <- obtener_cuenta_publica(anio)
}

# Verificamos si las descargas fueron exitosas :)
for (anio in 2013:2023) {
  if (!is.null(lista_cuentas_publicas[[as.character(anio)]])) {
    message("Datos para el año ", anio, " descargados con éxito.")
  } else {
    message("Datos para el año ", anio, " no disponibles.")
  }
}

# Para verificar una por una, podemos usar la siguiente línea de código:
print(head(lista_cuentas_publicas[["2021"]]))


###--- Ahora, queremos que cada base de datos tenga nombres de variables unificados
# para, posteriormente, poderlas unir.

# Vamos a pasar todos los nombres a mayúsculas
for (i in seq_along(lista_cuentas_publicas)) {
  names(lista_cuentas_publicas[[i]]) <- toupper(names(lista_cuentas_publicas[[i]]))  # Variables a mayúsculas
}

# Y a renombrar las variables para unificarlas. A todas las numéricas les vamos a poner "MONTO_"
for (i in seq_along(lista_cuentas_publicas)) {
  df <- lista_cuentas_publicas[[i]]
  
  # Verificar y renombrar columnas si existen
  if ("ADEFAS" %in% names(df)) {
    df <- df %>% rename(MONTO_ADEFAS = ADEFAS)
  }
  
  if ("EJERCICIO" %in% names(df)) {
    df <- df %>% rename(MONTO_EJERCIDO = EJERCICIO)
  }
  
  if ("MONTO_EJERCICIO" %in% names(df)) {
    df <- df %>% rename(MONTO_EJERCIDO = MONTO_EJERCICIO)
  }
  
  # Actualizamos la lista con el dataframe renombrado
  lista_cuentas_publicas[[i]] <- df
}

# Verificamos los cambios en las bases de datos
for (i in seq_along(lista_cuentas_publicas)) {
  print(names(lista_cuentas_publicas[[i]]))
}

###--------------------------------------------------- Unimos las bases de datos
# Esta es la unión para las Cuentas Públicas de 2013 a 2023

# Lista de variables deseadas. Nos interesan 32:
variables_deseadas <- c("CICLO", "ID_RAMO", "DESC_RAMO", "ID_UR", "DESC_UR", "GPO_FUNCIONAL", "DESC_GPO_FUNCIONAL", 
                        "ID_FUNCION", "DESC_FUNCION", "ID_SUBFUNCION", "DESC_SUBFUNCION", "ID_AI", "DESC_AI", 
                        "ID_MODALIDAD", "DESC_MODALIDAD", "ID_PP", "DESC_PP", "ID_OBJETO_DEL_GASTO", 
                        "DESC_OBJETO_DEL_GASTO", "ID_TIPOGASTO", "DESC_TIPOGASTO", "ID_FF", "DESC_FF", 
                        "ID_ENTIDAD_FEDERATIVA", "ENTIDAD_FEDERATIVA", "ID_CLAVE_CARTERA", "MONTO_APROBADO", 
                        "MONTO_EJERCIDO", "MONTO_MODIFICADO", "MONTO_DEVENGADO", "MONTO_PAGADO", "MONTO_ADEFAS")

# Prueba de preparación para unión: unificación de tipo de variables 
prueba <- lapply(lista_cuentas_publicas, function(df) {
  df$ID_RAMO <- as.integer(df$ID_RAMO)
  monto_cols <- grep("^MONTO_", names(df), value = TRUE)
  
  # Limpiar y convertir a numérico las columnas que empiezan con "MONTO_"
  df[monto_cols] <- lapply(df[monto_cols], function(col) {
    # Eliminar caracteres no numéricos, excepto el punto decimal y el signo negativo
    col_clean <- gsub("[^0-9.-]", "", col)
    as.numeric(col_clean)
  })
  
  # Asegurarse de que las columnas deseadas están presentes
  df[setdiff(variables_deseadas, names(df))] <- NA
  
  return(df)
})

# Unimos las bases de datos
cp1323 <- bind_rows(prueba)
# Nos quedamos sólo con las 32 variables que nos interesan
base_final <- cp1323 %>% select(all_of(variables_deseadas))

### Esta base se puede guardar en excel :)
write.xlsx(cp1323,  "cp1323.xlsx", rowNames = FALSE) 

#-------------------------------------------------------------------------------
# 02. Deflactar ---------------------------------------------------------------#
#-------------------------------------------------------------------------------

# Deflectar es un proceso esencial para poder comparar valores económicos de 
# diferentes períodos en términos reales, es decir, eliminando el efecto de la 
# inflación.

### Este es un deflactor que se va a subir a github para que esté disponible siempre
### Nota: la idea es automatizar el deflactor para que se actualice al año/periodo de tiempo correspondiente

deflactor <- read_csv("Downloads/deflactor.csv")


deflactor <- deflactor %>% 
  pivot_longer(cols = -Base, names_to = "ciclo", values_to = "deflactor") %>% 
  mutate(ciclo = gsub("=100", "", ciclo)) %>% 
  rename(CICLO = ciclo) %>% 
  mutate(CICLO = as.integer(CICLO))

# Merge con deflactor de año base
deflactor_24 <- deflactor %>% 
  filter(Base ==                               
rm(deflactor)
base_final <- left_join(base_final, deflactor_24, by = "CICLO")

# Deflactar montos

base_final <- base_final %>% 
  mutate(MONTO_APROBADO_def = ((MONTO_APROBADO)/(deflactor))*100,
         MONTO_MODIFICADO_def = ((MONTO_MODIFICADO)/(deflactor))*100,
         MONTO_DEVENGADO_def = ((MONTO_DEVENGADO)/(deflactor))*100,
         MONTO_PAGADO_def = ((MONTO_PAGADO)/(deflactor))*100,
         MONTO_ADEFAS_def = ((MONTO_ADEFAS)/(deflactor))*100,
         MONTO_EJERCIDO_def = ((MONTO_EJERCIDO)/(deflactor))*100)
rm(deflactor_ab, ab)

#-------------------------------------------------------------------------------
# 03. Capítulo, concepto, partida genérida y partida específica ---------------#
#-------------------------------------------------------------------------------

# Creación de variables
df <- base_final %>%
  mutate(ID_CAPITULO = (as.numeric(substr(ID_OBJETO_DEL_GASTO, 1, 1)))*1000,
         ID_CONCEPTO = (as.numeric(substr(ID_OBJETO_DEL_GASTO, 1, 2)))*100,
         ID_PARTIDA_GENERICA = (as.numeric(substr(ID_OBJETO_DEL_GASTO, 1, 3))),
         ID_PARTIDA_ESPECIFICA = (as.numeric(ID_OBJETO_DEL_GASTO))*1)

# "Muestrame el presupuesto y te digo tus prioridades"
...
# ID_CAPITULO a su respectiva descripción
cap <- function(ID_CAPITULO) {
...
df <- df %>%
  mutate(DESC_CAPITULO = cap(ID_CAPITULO))


# ID_CONCEPTO a su respectiva descripción

con <- function(ID_CONCEPTO) {
...
df <- df %>%
  mutate(DESC_CONCEPTO = con(ID_CONCEPTO))

# ID de partida genérica a descripción de partida genérica

pg <- function(ID_PARTIDA_GENERICA) {
...
df <- df %>%
  mutate(DESC_PARTIDA_GENERICA = pg(ID_PARTIDA_GENERICA))

# ID de partida específica a descripción de partida específica

pe <- function(ID_PARTIDA_ESPECIFICA) {
...
df <- df %>%
  mutate(DESC_PARTIDA_ESPECIFICA = pg(ID_PARTIDA_ESPECIFICA))
